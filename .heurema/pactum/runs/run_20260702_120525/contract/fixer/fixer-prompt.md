...
Current contract version: 9bf8e2a00bb85c2966829e2d10b2ce02b3786ca10138576317c4f95736a1eebf
...
1. [codex-xhigh/testability] Several mandatory SPEC.md documentation requirements are not backed by validation commands.
   Evidence: Acceptance requires documenting the full manifest schema, signature.sig over raw manifest bytes, files[].sha256 transitive binding, GET /challenge?fpr={fpr}, all three auth headers, and status codes 201/401/410/413/429, but validation only greps for a subset: '"ECHO"', absence of 'ECHS'/'project_hint', sender_echo_id, sender_fingerprint, created_at, 'GET /challenge', 'X-Echos-Nonce', '413', and '410'.
2. [codex-xhigh/testability] The Codex dead-code acceptance criterion depends on non-runnable code review instead of a concrete validation command.
   Evidence: Acceptance says this is 'Verified by go vet ./... passing and the code review confirming no unreachable branches remain'; the validation commands include go vet, but no command that checks the specific deleted branches in internal/session/codex.go.
3. [codex-xhigh/validation-soundness] The negative SPEC.md checks use `grep -L`, which is not a sound absent-pattern gate command here.
   Evidence: Validation commands include `grep -L 'ECHS' SPEC.md` and `grep -L 'project_hint' SPEC.md`.
...
- List fields are whole-list replacements in contract revise: if you include a list field, include every item that should remain, including unchanged items.
...
```json
{
  ...
  "base_version": "9bf8e2a00bb85c2966829e2d10b2ce02b3786ca10138576317c4f95736a1eebf",
  "contract": {
    "scope": {
