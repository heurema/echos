...
**Scope in**:
  ...
  - Core verbs: `echos send <friend> [session-id]` (default session = latest in the current cwd project) uploads an envelope addressed to the friend's fingerprint and prints a success line with TTL; `echos inbox` lists pending items; `echos open [id] [--dir]` (default = newest inbox item, dir=cwd) decrypts, verifies the sender against the address book, installs native files, and prints the resume command.
